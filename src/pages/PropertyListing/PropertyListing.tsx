...
function PropertyListing({
  ...
  const [activeFilter, setActiveFilter] =
    useState<(typeof listingFilters)[number]>("All");
  const [advancedFilters, setAdvancedFilters] =
    useState<PropertyFilters | null>(null);
  ...
  useEffect(() => {
    ...
    listProperties({
      verifiedOnly:
        activeFilter === "Verified only" ? true : undefined,
      bedrooms:
        activeFilter === "1 Bedroom"
          ? 1
          : activeFilter === "2 Bedroom"
            ? 2
            : undefined,
  ...
  return (
    <div style={{ padding: '5px' }}>
      ...
      {isLoading && (
        <p className="mt-6 px-4 text-sm text-muted">
          Loading properties…
        </p>
      )}

      {error && (
        <p className="mt-6 px-4 text-sm text-error-600">
          {error}
        </p>
      )}

      {favoriteError && (
        <p className="mt-2 px-4 text-sm text-error-600">
          {favoriteError}
        </p>
      )}
