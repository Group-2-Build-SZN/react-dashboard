...
export function listSavedProperties(listingPurpose?: 'rent' | 'sale') {
  return api
    .get<ApiEnvelope<{ savedAt: string; property: ApiPropertyDetail }[]>>(
      `/saved${listingPurpose ? `?listingPurpose=${listingPurpose}` : ''}`
    )
    .then((r) => r.data);
}

export function submitInquiry(propertyId: string, message: string) {
  return api.post(`/properties/${propertyId}/inquiries`, { message });
}

export function reportProperty(propertyId: string, formData: FormData) {
  return api.post(`/properties/${propertyId}/report`, formData);
}