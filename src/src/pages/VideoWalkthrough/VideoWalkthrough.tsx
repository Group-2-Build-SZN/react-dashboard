...
function VideoWalkthrough({ propertyId, isPremium, onBack, onUnlockContact }: VideoWalkthroughProps) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      ...
      <div className="mt-4">
        {isPlaying && property.videoUrls[0] ? (
          <video
            src={property.videoUrls[0]}
            controls
            autoPlay
            className="mx-4 h-56 w-[calc(100%-2rem)] rounded-2xl bg-black object-cover"
          />
        ) : (
          <VideoPreview
      ...
      {showUnlockPrompt && (
        ...
        >
          ...
          >
            <h3 className="text-lg font-bold text-gray-900">Unlock to watch</h3>
            <p className="mt-2 text-sm text-gray-500">
              The video walkthrough is part of My Ulo Premium. Unlock contact &amp; videos to
              watch it.
            </p>
            <div className="mt-4 flex flex-col gap-2">
              <Button
                ...
                onClick={() => {
                  ...
                  onUnlockContact?.(property.id);
                }}
              >
                Unlock Now
              </Button>
              <Button variant="outline" className="w-full" onClick={() => setShowUnlockPrompt(false)}>
                Not now
              </Button>
            </div>
          </div>
